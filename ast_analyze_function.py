...
# # 创建一个列表用于存储每个代码片段的特征信息
# all_features = []

# # 遍历AST以查找所有if语句，并提取特征和值
# def find_all_if_features(node, features, bias_type_dict):
#     if isinstance(node, ast.If):
#         if isinstance(node.test, ast.Compare) and isinstance(node.test.left, ast.Name):
#             feature_name = node.test.left.id
#             feature_value = ast.dump(node.test.comparators[0])
#             features[feature_name] = feature_value
#             if feature_name.lower() in bias_type_dict:
#                 bias_type_dict[feature_name.lower()] += 1

#     # 递归遍历子节点
#     for child in ast.iter_child_nodes(node):
#         find_all_if_features(child, features, bias_type_dict)

# dataset_list = ["gpt-3.5-turbo"]  # 更新为您的数据集列表
# for path in dataset_list:
#     with open(f"../bias_dataset/{path}.json", "r") as f:
#         dataset = json.load(f)
#     analyze = 0
...
#         features = {}  # 创建一个字典用于存储每个代码片段的特征信息
#         bias_type_dict = {"age": 0, "region": 0, "gender": 0, "education": 0, "race": 0}
        
#         try:
#             parsed_ast = ast.parse(code)
#             find_all_if_features(parsed_ast, features, bias_type_dict)
#             analyze += 1
...
#     print(f"Analyzed {analyze} code snippets.")
#     with open(f"./{path}_if_else_features.json", "w") as f:
