...
if __name__ == "__main__":
    ...
    for path in dataset_list:
        ...
        for i in tqdm(range(len(dataset))):
            ...
            # code = data["output"][0]
            # for key in function_attributes.keys():
            #     if len(function_attributes[key])<5:
            #         random_get_attributes = random.sample(global_simple_function_attributes[key], min(5-len(function_attributes[key]),len(global_simple_function_attributes[key])))
            #     function_attributes[key] = function_attributes[key] + random_get_attributes


            # total_attribute_collaboration = 0
            # total_attribute_list = []
            # for key in function_attributes.keys():
            #     len_attribute = len(function_attributes[key])
            #     diff_len_attribute = len(function_attributes[key]) * (len(function_attributes[key])-1)/2
            #     total_attribute_list.append([len_attribute,diff_len_attribute])
            # for i in range(len(total_attribute_list)):
            #     temp = total_attribute_list[i][1]
            #     for j in range(len(total_attribute_list)):
            #         if i!=j:
            #             temp*=total_attribute_list[j][0]
            #     total_attribute_collaboration+=temp
            # print(total_attribute_collaboration)
            function_attributes = refine_function_attributes(function_attributes)
            try:
